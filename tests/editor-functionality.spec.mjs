...
test('Create new post with all tag types', async ({ page }) => {
...
test('Edit existing post preserves data', async ({ page }) => {
  await page.goto('http://localhost:3000/posts');
  
  // Find a draft post to edit
  ...
  await editPage.waitForSelector('input[placeholder="Post title"]', { timeout: 10000 });
  ...
  await editPage.waitForSelector('.ProseMirror', { timeout: 10000 });
...
test('Post list shows correct timestamp formatting', async ({ page }) => {
  await page.goto('http://localhost:3000/posts');
...
test('Tag dropdowns appear in correct order', async ({ page }) => {
  await page.goto('http://localhost:3000/posts');
